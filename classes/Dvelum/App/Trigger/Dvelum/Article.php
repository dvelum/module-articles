...
namespace Dvelum\App\Trigger\Dvelum;

use Dvelum\App\Trigger;
use Dvelum\Orm\Model;
use Dvelum\Orm\RecordInterface;

class Article extends Trigger
{
    public function onAfterPublish(RecordInterface $object)
    {
        parent::onAfterPublish($object);
        $this->clearTopCache($object);
    }

    public function onAfterUnpublish(RecordInterface $object)
    {
        parent::onAfterUnpublish($object);
        $this->clearTopCache($object);
    }

    public function onAfterDelete(RecordInterface $object)
    {
    ...
    public function clearTopCache($object)
    {
        ...
        /**
         * @var \Dvelum\App\Model\Dvelum\Article $model
         */
        $model = Model::factory('Dvelum_Article');

        $this->cache->remove($model->getCacheKey(['item', 'url', $object->url]));
